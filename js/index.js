$(document).ready(function() {
  var colors =['#454545','#3b3082','#7a35a4','#32523b','#4f4b35','#1d1a1c'];
  ...
  function quoteReady(newQuote) {  
    ...
    if (quote && author) {
       $('#quote-text').replaceWith('<blockquote class="large" id="quote-text">' + quote + '</blockquote>');            
       $('#author').replaceWith('<cite id="author">' + author + "</cite>");
       
       $('.twitter-share-button').attr("href", "https://twitter.com/intent/tweet?text=" + tweet);
      
      $('.tumblr-share-button').attr("href", "http://www.tumblr.com/share/link?url=" + encodeURIComponent(document.URL) + "&name=Quote&description=" + tweet);
      
       var color = colors[getRandomInt(0,colors.length)];
    //$('body,#newQuote,#button-area a').css('background-color', color);
      //$('#quote-text,#author').css('color',color);      
      $("body").animate({
        backgroundColor: color
       
      }, 1500);
      $("#newQuote,#button-area a").animate({
        backgroundColor: color
      }, 1500);
